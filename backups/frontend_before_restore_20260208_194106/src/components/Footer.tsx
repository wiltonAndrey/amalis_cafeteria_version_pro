...
const Footer: React.FC = () => {
  ...
  const socialLinks = [
    ...
    { name: 'Twitter', icon: TwitterIcon, href: settings.socialLinks?.twitter },
  ].filter(link => link.href);

  const contactoLinks = [
    ...(settings.address ? [{ name: settings.address, href: '/#location' }] : []),
    ...(settings.phone ? [{ name: settings.phone, href: `tel:${settings.phone}` }] : []),
    ...(settings.email ? [{ name: settings.email, href: `mailto:${settings.email}` }] : []),
  ];

  const hoursText = (settings.openingHours || '').trim();
  const splitByDot = hoursText.includes('·') ? hoursText.split('·') : hoursText.split('Â·');
  const dayPart = (splitByDot[0] || '').trim();
  const timePart = (splitByDot[1] || '').trim();
