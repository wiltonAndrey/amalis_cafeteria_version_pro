      ...
      <Reveal width="100%">
        <div className="bg-brownie rounded-[3.5rem] overflow-hidden flex flex-col lg:flex-row shadow-[0_32px_80px_-20px_rgba(0,0,0,0.7)] group/section transition-all duration-700 min-h-[650px] border border-white/5">
          ...
          <div className="lg:w-5/12 p-10 lg:p-20 flex flex-col justify-center text-beige relative overflow-hidden bg-gradient-to-br from-brownie to-black/40">
            <div className="absolute top-0 left-0 w-full h-full opacity-[0.07] pointer-events-none" aria-hidden="true">
            ...
            <div className="space-y-12 relative z-10">
              ...
              >
                {loading ? (
                  <div className="w-6 h-6 border-2 border-brownie border-t-transparent rounded-full animate-spin" role="status"><span className="sr-only">Cargando ubicación</span></div>
                ) : (
                  <svg className="w-6 h-6 transform group-hover/btn:rotate-12 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <div className="pt-10 border-t border-white/10">
                <div className="flex items-center gap-4 mb-6 group/addr">
                  ...
                  <div>
                    <h3 className="font-bold text-xl font-serif text-white tracking-wide">Nuestra Casa</h3>
          ...
          <div ref={mapRef} className="lg:w-7/12 min-h-[500px] lg:h-auto relative overflow-hidden flex items-center justify-center p-6 lg:p-16">
            {/* Stylized Google Map - lazy loaded */}
            <div className="absolute inset-0 z-0 bg-brownie">
              {showMap && (
                <iframe
                  className="w-full h-full opacity-60 grayscale invert contrast-[1.1] mix-blend-luminosity brightness-90"
                  style={{ border: 'none', margin: 0, overflow: 'hidden' }}
                  loading="lazy"
                  src="https://maps.google.com/maps?q=38.19156,-0.55558&t=m&z=16&output=embed&iwloc=near"
                  title="Ubicación de Amalis Cafetería en Santa Pola"
                  aria-label="Mapa de ubicación de Amalis Cafetería"
                ></iframe>
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-brownie via-brownie/20 to-transparent opacity-40 pointer-events-none"></div>
            </div>

            <div
              className="bg-black/60 backdrop-blur-xl p-10 lg:p-12 rounded-[3.5rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8),0_0_80px_-10px_rgba(196,167,125,0.2)] w-full max-w-md relative z-20 border border-white/10 animate-float"
            >
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="font-serif font-bold text-4xl text-white mb-4 tracking-tight leading-tight">
                ...
                <div className="w-full group/tel">
                  <a
                    ...
                    className="w-full flex items-center justify-center gap-3 bg-white text-brownie py-6 rounded-2xl font-bold text-2xl hover:bg-caramel hover:text-white transition-all duration-300 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.4)] hover:shadow-[0_25px_50px_-12px_rgba(196,167,125,0.5)] hover:scale-[1.03] active:scale-95 mb-6"
                  >
                    <span>{LOCATION_INFO.contactCard.ctaLabel}</span>
                    <span className="text-caramel group-hover/tel:text-white transition-colors" aria-hidden="true">→</span>
                  </a>
                  <div className="relative group/num inline-block">
                    <p className="text-white font-bold tracking-widest text-2xl font-mono">
                      {LOCATION_INFO.phone}
                    </p>
