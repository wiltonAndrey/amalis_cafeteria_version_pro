...
const AnimatedStars: React.FC = () => {
  ...
  return (
    <div
      ref={containerRef}
      className={`text-caramel mb-5 flex justify-center gap-1.5 pt-4 ${isVisible ? 'stars-playing' : 'stars-paused'}`}
    >
      {STAR_IDS.map((starId) => (
        <svg
          key={starId}
          className="star-animate w-5 h-5 fill-current drop-shadow-sm"
          viewBox="0 0 20 20"
          aria-hidden="true"
        >
